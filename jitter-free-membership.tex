...
without sacrificing consistency.  This works because\footnote{TODO nonsequitur}
if the invariants of appendix \ref{synod-safety} are satisfied with a value
function $v$ then they are satisfied for any value function $v'$ that agrees
with $v$ on sent proposals, i.e. having $v(p) = v'(p)$ whenever $\prop{}{p}$.
Furthermore only $\owner{p}$ may propose $p$, so if it has not yet proposed a
value for $p$ it can, in effect, freely modify $v(p)$ as desired.
...
available. For instance, the Cheap Paxos algorithm implements $v$ using only
the $f+1$ primary processors and none of the auxiliary processors, representing
$v$ as an optimised version of a replicated insert-only set \cite{crdts} of
pairs $\{ \langle p, v(p) \rangle \mid \prop{}{p} \}$.
...
the nodes could be used as a quorum: roughly speaking\footnote{TODO have not
defined these yet}, \[Q^\textrm{I}(p_1) = Q^\textrm{II}(p_2) = \{ q \mid q
\text{ is a majority}\}\] was a constant, and $Q^\textrm{I}(p_1) \frown
Q^\textrm{II}(p_2)$ for all $p_1$ and $p_2$ since all majorities intersect.
This intersection property is stronger than needed, and the ability to vary the
set of quorums by proposal and by phase is the key ingredient in being able to
perform jitter-free membership changes. The proof of theorem
...
p_2$ and then without loss of generality that ${p_2 \prec p_1}$. By invariant
\ref{synod-chosen} there is a quorum $q \in Q^\textrm{II}(p_1)$ such that
$\acc{}{a}{p_1}$ for every node $a \in q$. Since $q$ is nonempty, by invariant
\ref{synod-acc} it must be that $\prop{}{p_1}$ was previously sent.  Therefore
by lemma \ref{synod-lemma} it follows that $v(p_1) = v(p_2)$ as required.
\end{proof}
