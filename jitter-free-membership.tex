...
each proposal $p$ has a unique node $\owner{p}$ that may send out $\prop{p}{i}$
and which is therefore responsible for the value $v_i(p)$ for each $i$.
...
$\chosen{i-1}{p'}$ for some $p'$, so that $i < i_\mathrm{max}$, and \item there
is a quorum $q \in Q_{e(i)}$ such that $\acc{i}{a}{p}$ has been received for
