...
phase II.  More formally, a node may emit $\prop{i}{b}$ only if $e(b) \le e(i)$
and it has received promises for ballot $b$ in instance $i$ from a quorum in
$\QI{e(b)}$. A node may decide $\chosen{i}{b}$ only if ${e(i) \le e(b)+1}$ and
it has received $\acc{i}{a}{b}$ from a quorum in $\QII{e(i)}$.  Thus if
