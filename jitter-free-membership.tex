...
I; this is shown to preserve consistency as long as the configuration change
satisfies certain conditions as described in section
...
connectivity) because such infrastructure may be expensive to replicate. In
more detail, a na\"ive approach to replacing a node in a three-node cluster
would be to perform this sequence of configuration changes:
\[\begin{array}{rcccc}
%
...
to rely on independent infrastructure, because it can only handle a single node
failure throughout. It also has no node with a casting voite in era $e+1$. On
the other hand the following sequence requires only three separate sets of
infrastructure and both $a_1$ and $a_2$ have casting votes throughout:
\[\begin{array}{rcccc}
%
\textrm{node}&a_{\textrm{old}}&a_{\textrm{new}}&a_1&a_2 \\
%
...
$Q_{e}$ to an unrelated configuration $Q'$ it is possible to set $Q_{e+2} = Q'$
and in the interim set $Q_{e+1}$ to be the \textit{joint configuration} of
$Q_{e}$ and $Q'$: \[Q_{e+1} = \{ q \cup q' \mid q \in Q_{e}, q' \in Q' \}.\]
Intuitively, while this interim configuration is in use all decisions must be
agreed by quorums of both the old and new configuration. Certainly this
satisfies that \[Q_{e} \frown Q_{e + 1} \frown Q_{e+1} \frown Q_{e + 2} \frown
Q_{e + 2}\] as required.
...
configurations, but those of $s_3$ and $s_5$ are not.

If $w(a) = 0$ for all
$a$ then $w$ is said to be \textit{weightless} and $M(w) = \varnothing$.


Here it is demonstrated that the configuration changes supported by this
technique generalise those of Raft and of Dynamic Paxos, and can also be used
to stop a running cluster.


  If $Q_e$ and $Q_{e+1}$ are nonempty then intuitively the
...
removing a single node is always a sufficiently small reconfiguration.  %
