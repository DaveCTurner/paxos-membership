...
approach described here allows phase II to start using a new membership without
re-running phase I; this is shown to preserve consistency as long as the
membership change is sufficiently small (in a sense which is made precise in
section \ref{types-of-membership-change} below.) By removing phase I from the
critical path of a membership change the possibility of jitter caused by a
...
may be desirable to be able to deduce $\chosen{i}{p}$ even if the chosen value
at instance $i-1$ is not yet known. There must be a limit $\kappa > 0$ on how
many instances may be chosen concurrently, in case any of the unchosen
instances eventually chooses a membership change. Note carefully that the
concurrency parameter $\kappa$ is different from the parameter $\alpha$ in the
original Dynamic Paxos: there $\prop{i}{p}$ may not occur until all instances
$\le i - \alpha$ have been chosen whereas here there is no limit on which
proposals can be made and the only restriction is that $\chosen{i}{p}$ may not
occur until all instances $\le i - \kappa$ are chosen. In the common case that
the transitions may not be reordered, $\kappa = 1$ is sufficient.
...
\item \label{synod-quorums} For $p_1 \succ p_2 \in \mathbb
P$ there are memberships $Q^\textrm{I}(p_1)$ and $Q^\textrm{II}(p_2) \subseteq
\mathcal P \mathbb A$ such that if $\prop{}{p_1}$ and $\chosen{}{p_2}$ then
...
\item \label{synod-prop} $\prop{}{p}$ only if there is a quorum
$q^\textrm{I} \in Q^\textrm{I}(p)$ such that for every node $a \in q^\textrm{I}$
either $\fprom{}{a}{p}$ or else $\exists p'.  \bprom{}{a}{p}{p'}$, and if ${P = \{
p' \mid \exists a \in q^\textrm{I}. \bprom{}{a}{p}{p'} \} \ne \varnothing}$
then it follows that $v(p) = v(\mathrm{max}(P))$.
...
\begin{itemize}
\item for each $j \le i - \kappa$ there is a $p'$ with $\chosen{i-1}{p'}$,
\item $e(i) \le e(p) + 1$, and
\item there is a quorum $q \in Q_{e(i)}$ with $\acc{i}{a}{p}$ for every $a \in
q$.
\end{itemize}

\end{enumerate}

\begin{lemma}\label{paxos-synod-quorum-invariant} For $p_1
\succ p_2 \in \mathbb P$ and instance $i$, if $\prop{i}{p_1}$ and
$\chosen{i}{p_2}$ then ${Q_{e(p_1)} \frown Q_{e(i)}}$.  \end{lemma}
