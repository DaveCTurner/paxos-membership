...
\usepackage[paperwidth=210mm,paperheight=150mm]{geometry} % TODO just for on-screen
%
...
The value of the delay parameter $\alpha$ may vary while the system is running,
so it is certainly possible to design a mechanism for choosing a reasonably
suitable value and adapting it to changing system conditions. The approach
described here shows an alternative technique which runs the new phase I
without needing to fix the instance at which the membership change takes
effect. As this technique imposes no limit on how soon the new phase I must be
completed, it entirely avoids the possibility of jitter caused by a pipeline
stall.
...
small, jitter-free changes but it is also shown that this technique generalises
that of Dynamic Paxos so 
...
messages to each other. Communication is asynchronous, in the sense that each
messages may be delayed, reordered, duplicated or dropped.
