...
\varnothing$ then no value can ever be chosen for $i$, but empty memberships
can be skipped by letting $e(i) = e(i-1) + 2$. If $Q_{e(i)-1} = \varnothing$
then the consecutive epoch intersection property is trivial and $Q_{e(i-1)}$
and $Q_{e(i)}$ can be unrelated. Note that if an epoch is to be skipped like
...
the sequence, $Q_0, \ldots, Q_{e_\mathrm{max}}$, and whose transitions append
one or more memberships to this sequence. Similarly, the epoch numbers are
defined by a state machine whose state is the sequence of values $e(0), \ldots,
e(i_\mathrm{max})$ and whose transitions append one or more values to this
sequence. The values are chosen so that at instance $i$ the epoch-number state
machine satisfies $i_\mathrm{max} > i$, and similarly $e_\mathrm{max} \ge
