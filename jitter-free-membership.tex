...
If the pipeline is too long then a reconfiguration may take a long time to
complete. On the other hand if the pipeline is too short then it may stall when
the system is being reconfigured which can be observed as jitter in the rate
that the system makes progress. This paper shows that the algorithm can be
modified to support dynamic reconfigurations with an unbounded pipeline as long
as the reconfigurations are sufficiently small, and gives a simple
characterisation of a set of sufficiently small reconfigurations that is useful
in practice. It also shows that an arbitrary reconfiguration can be performed
in a single step by temporarily limiting the pipeline length.\end{abstract}
...
relatively rare periods of maintenance. The more obvious migration process:
\[\begin{array}{rcccc}
\textrm{node}&a_{\textrm{old}}&a_{\textrm{new}}&a_1&a_2 \\
w_e&1&0&1&1\\
w_{e+1}&1&1&1&1\\
w_{e+2}&0&1&1&1\\
\end{array}\]
requires four zones in epoch $e+1$ to ensure resilience to a zone-wide failure.
