...
M(w)$ then $q \cap q' = \varnothing$. If $w(a) = 0$ for all $a$ then $w$
is said to be \textit{weightless} and $M(w) = \varnothing$.
...
that $q^\textrm{I} \cap q^\textrm{II} \ne \varnothing$. There are three simple
ways to ensure this property holds:

\begin{enumerate}

\item If either $w_{e-q}$ or $w_e$ is weightless then the property trivially
holds.

\item If $w_{e-1}$ and $w_e$ differ by a constant factor, in the sense that
there are integers $m$ and $n$ such that $m w_{e-1}(a) = n w_e(a)$ for all $a$,
then $M(w_{e-1}) = M(w_e)$ and hence the property holds.
...
Recall that Paxos is designed to be resilient to $\le f$ failures, which can be
achieved if each node always holds $< \frac{1}{2f}$ of the total weight. This
resilience can also be used to perform routine maintenance on the nodes by
means of a \textit{rolling upgrade} where each node is taken offline and
upgraded in turn. During such a process there is only ever one node offline for
maintenance and therefore the system remains available.

However, a na\"ive approach to perform a rolling upgrade puts the system at risk
during the upgrade: if $f = 1$ and one of the nodes is down for maintenance then
the failure of another node would render the system unavailable. In terms of
weights:
\begin{tabular}{rccc}
&$A$&$B$&$C$ \\
1.&1&1&1\\
...
3.&1&1&1
\end{tabular}
On line 2, node $A$ has been taken down for maintenance so both $B$ and $C$
have $\ge \frac{1}{2}$ of the total weight which means the system is
temporarily not resilient to any failures. A common solution to this problem is
simply to provision for more failures than are actually requir
...
taken offline for maintenance for a period of time

Assuming that node failures are independent, the probability of a cluster-wide
failure appears to be extremely small. However, in a real-world system node
failures are not independent: certain sets of nodes may share critical
...
A common use for changing the membership of the cluster is to perform
a rolling upgrade of a node remove a node
