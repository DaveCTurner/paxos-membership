...
ensure safety during reconfiguration. Raft imposes no such limit on concurrent
requests and preserves safety by insisting on sufficient similarity between the
configurations before and after the reconfiguration. It is shown that Paxos can
be extended to support reconfiguration without limiting concurrency in much the
same way as is done in Raft.\end{abstract}
...
Q^\textrm{II}(b_2)$ only when $\prop{}{b_1}$, $\chosen{}{b_2}$ and $b_1 \succ
b_2$.
