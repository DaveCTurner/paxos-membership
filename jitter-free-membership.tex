...
TODO\footnote{Cheap Paxos} observes that these values can be replaced in some
...
$v$ then they are satisfied for any value function $v'$ having $\prop{}{p}
\implies v(p) = v'(p)$.  Furthermore only $\owner{p}$ may propose $p$, so if it
has not yet proposed a value for $p$ it can, in effect, freely modify $v(p)$ as
desired.
...
and has received $\acc{i}{a}{p}$ from a quorum $q^\textrm{II} \in Q_{e(i)}$.
...
emphasising the subtle point that nodes use the membership $Q_{e(p)}$
corresponding to the proposal $p$ for phase I, but the membership $Q_{e(i)}$ of
the instance $i$ for phase II.

\subsection{Types of membership change}

There are two qualitatively different kinds of membership change that satisfy
the consecutive epoch intersection property. The first is where $Q_{e-1}$ and
$Q_e$ are nonempty, then the property is satisfied as long as the memberships
are sufficiently similar. For example, if \[Q_{e-1} = \{ \{ a_0, a_1 \}, \{
a_0, a_2 \}, \{ a_1, a_2 \}, \{ a_0, a_1, a_2 \} \}\] is the set of all
majority subsets of $\{ a_0, a_1, a_2 \}$ and \[\begin{aligned}Q_e
    &=& \{ \{ a_0, a_1, a_2 \}, \{ a_0, a_1, a_3 \}, \\&&
    \{ a_0, a_2, a_3 \}, \{ a_1, a_2, a_3 \}, \{ a_0, a_1,
a_2, a_3 \}\}\end{aligned}\] is the set of all majority subsets of $\{a_0, a_1, a_2, a_3\}$
then these are sufficiently similar, in the sense that they are nonempty and
satisfy the consecutive epoch intersection property.


\subsection{Defining new memberships and epochs}

As in Dynamic Paxos, the memberships $Q_0, Q_1, \ldots$ and epoch numbers
$e(i)$ are themselves chosen by Paxos.
...
cluster. Recall that this sequence of
