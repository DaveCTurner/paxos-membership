...
the consecutive epoch intersection property. If $Q_{e-1}$ and $Q_e$ are both
nonempty then they can satisfy the property by being sufficiently similar. For
example, if the nodes are numbered $a_1, a_2, \ldots$ then let $s_n = \{a_1,
\ldots, a_n\}$ and let $\maj{s} = \{ q \subseteq s \mid 2 |q| > |s| \}$ be the
set of all majority subsets of $s$. Then $\maj{s_3}$ and $\maj{s_4}$ are
