...
\owner{p'}$, and $\owner{p}$ should try and receive a quorum of promises for
$p'$. It can do this by dividing the cluster into two quorums in such a way
that it has a \textit{casting vote} between the two quorums: it finds $q \in
Q_{e(p)}$ and $q' \in Q_{e(p)+1}$ having $q \cap q' = \{\owner{p}\}$ and
broadcasts $\prep{p'}$ to all nodes in $q'$ except itself. While it is waiting
for the promises to arrive in response to this broadcast it continues to make
progress choosing instances using the quorum $q \in Q_{e(p)}$.  Eventually when
promises have arrived from every node in $q'$ except itself, it may make a
promise to itself, immediately complete phase I for proposal $p'$, and start
broadcasting proposals $\prop{i}{p'}$. This restores the system to normal
running.
...
Notice also that this process requires $\owner{p}$ to be able to divide the
cluster so that it has a casting vote. If this is not possible, it may be able
to \textit{abdicate} to another node that does have a casting vote.
