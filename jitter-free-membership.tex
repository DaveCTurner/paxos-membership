...
of the Synod algorithm: whenever $\chosen{}{p_1}$ and $\chosen{}{p_2}$ then
$v(p_1) = v(p_2)$. The full details of the conditions under which nodes can
send these messages is included in appendix \ref{synod-safety}.
...
a quorum of nodes then the state machine $S_\infty$ has completed phase I at
...
defining the value for each proposal, and theorem \ref{paxos-safety} shows that
whenever $\chosen{i}{p}$ and $\chosen{i}{p'}$ it follows that $v_i(p) =
v_i(p')$.
...
$\chosen{i}{p_2$ then $v_i(p_1) = v_i(p_2)$.
\end{theorem}
