...
complete, but if it too short then it may stall when the system is being
reconfigured which can be observed as jitter in the rate that the system makes
progress. This paper shows that the algorithm can be modified to support
dynamic reconfigurations with an unbounded pipeline as long as the
reconfigurations are sufficiently small, and gives a simple characterisation of
a set of sufficiently small reconfigurations that is useful in practice. It
also shows that an arbitrary reconfiguration can be performed in a single step
by temporarily limiting the pipeline length.\end{abstract}
