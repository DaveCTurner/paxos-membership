...
This presentation concludes with a simple characterisation of a useful set of
sufficiently small membership changes, along with examples showing how
sequences of these small changes can be used to achieve common administrative
tasks.
...
Clearly each membership $Q_e$ in the sequence must satisfy that every $q, q'
\in Q_e$ have nonempty intersection in order to satisfy the consecutive epoch
intersection property. In addition, every $q^\textrm{I} \in Q_{e-1}$ and
$q^\textrm{II} \in Q_e$ must have nonempty intersection. There are two useful
ways that this can be achieved.

Firstly, note that this property holds (vacuously) if either $Q_{e-1} =
\varnothing$ or $Q_e = \varnothing$. Clearly if there is an instance $i$ such
that $Q_{e(i)} = \varnothing$ then no value can ever be chosen for $i$, but
epochs may be skipped by letting $e(i) = e(i-1) + 2$ at some instance $i$. By
doing so, and having the skipped membership $Q_{e(i)-1} = \varnothing$ then the
consecutive epoch intersection property holds for any memberships $Q_{e(i-1)}$
and $Q_{e(i)}$.  Note that if an epoch is to be skipped like this then phase I
must be re-run for a new proposal $p'$ having $e(p') \ge e(i)$, since if phase
I is complete at proposal $p$ then $e(p) \le e(i-1) < e(i) - 1$ and hence
$\neg\chosen{i}{p}$. Thus an empty membership and an epoch skip can be used to
perform arbitrary membership changes as in Dynamic Paxos, but this requires a
phase I to be performed on the critical path which risks jitter if it is not
completed quickly enough.

\def\maj#1{\mathbf{maj}(#1)}

Secondly, if $Q_{e-1}$ and $Q_e$ are sufficiently similar then they may satisfy
the consecutive epoch intersection property.  Certainly if $Q_{e-1} = Q_e$ then
the property holds. For a less trivial example, suppose $\mathbb A = \{ a_1,
a_2, \ldots \}$, let $s_n = \{a_1, \ldots, a_n\}$ and let $\maj{s} = \{ q
\subseteq s \mid 2 |q| > |s| \}$ be the set of all majority subsets of $s$.
Then $\maj{s_3}$ and $\maj{s_4}$ satisfy the consecutive epoch intersection
property, as do $\maj{s_4}$ and $\maj{s_5}$, but $\maj{s_3}$ and $\maj{s_5}$ do
not because $\{a_1, a_2\} \in \maj{s_3}$ and $\{a_3, a_4, a_5\} \in \maj{s_5}$
do not intersect.
...
sequence of memberships used by a cluster.  Recall that this sequence of
...
Corollary \ref{weights-equal} shows that if $q^\textrm{I} \in M(w_e)$ then
$q^\textrm{I} \cap q^\textrm{II} \ne \varnothing$, so it remains to show the
property where $q^\textrm{I} \in M(w_{e-1})$. There are three simple ways to
ensure this:

\begin{enumerate}

\item If $w_{e-1}$ is weightless then the property holds vacuously.
...
all $a$, then $M(w_{e-1}) = M(w_e)$ and hence the property holds.
...
property holds even if $w_e$ and $w_{e+2}$ are completely unrelated. As shown
in section \ref{types-of-membership-change} above, the problem with skipping
epochs like this is that it risks causing jitter, whereas if no epochs are
skipped then jitter can be avoided.
...
them share infrastructure then the system is at risk of a zone-wide failure in
epoch $w_{e+1}$. On the other hand, arranging for four zones is likely to be
much harder than arranging for three\footnote{For instance, at time of writing,
  only one Amazon Web Services region (\texttt{us-east-1}) has four independent
zones, whereas four of them have three: \texttt{ap-southeast-2},
\texttt{eu-west-1}, \texttt{sa-east-1} and \texttt{us-west-2}.}.  Fortunately,
it is possible to safely replace a node within a single zone
$z_{\textrm{maint}}$ using only three zones and the following sequence of
operations.
\[\begin{array}{rcccc}
