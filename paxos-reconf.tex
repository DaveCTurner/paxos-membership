...
?~? Draft \gitid}%TODO Journal of \LaTeX\ Class Files,~Vol.~14, No.~8, August~2015}%
{Turner: \mytitle}
...
deterministic state machine is replicated across a set of distinct nodes. Being
deterministic, the nodes' states remain synchronised if they all start in the
same state and perform the same sequence of transitions.

In order to arrange for each node to perform the same transitions the system
must achieve consensus on (or \textit{choose}) a sequence $v_0, v_1, \ldots$ of
...
reconfigurations that may occur to only allow ones that are sufficiently minor
that they cannot result in inconsistency. An operator may then then perform a
...
up-front, but there seems to be no published method for doing so whilst
ensuring consistency. It would be better still to be able to impose this limit
only when a configuration change is in progress and allow an unbounded pipeline
while the configuration is static. The need for any such limit seems
...
proofs of liveness and consistency are included in section \ref{liveness} and
appendices \ref{synod-safety} and \ref{paxos-invariants} and differences from
the original are highlighted throughout. The appendices are informal versions
of the formal proofs performed using the Isabelle/HOL proof assistant
\cite{isabelle-hol}.
...
the reconfigurations that can be performed to be sufficiently minor that
...
\begin{figure*}[t!]
  \label{synod-invariants-figure}
\caption{Invariants preserved by the Synod algorithm}
...
The system satisfies a set of invariants listed in figure
...
\ref{synod-safety}. The extra flexibility in quorums, and the removal of values
from messages, are the two main differences between this theorem and the
corresponding ones in earlier work.
...
properties.  To recover liveness, note that if the invariants of appendix
\ref{synod-safety} are satisfied with a value function $v$ then they continue
to be satisfied if $v$ is replaced by another value function $v'$ that agrees
with $v$ on proposed ballots, i.e. where $v(b) = v'(b)$ if $\prop{}{b}$ but not
necessarily otherwise. Since only $\owner{b}$ may propose $b$, if it has not
yet itself proposed a value for $b$ it can deduce that $\neg\prop{}{b}$ and
therefore freely modify $v(b)$.
...
algorithm becomes small enough that it needs no special treatment and a more
efficient consensus-free technique may be used to implement $v$ efficiently.
...
\ref{synod-safety}. This weaker invariant is the key to allowing more general
reconfigurations to take place safely as described in section
\ref{configuration-changes} below.

\section{The Paxos algorithm}\label{paxos-text}

\begin{figure*}
  \label{paxos-invariants-figure}
\caption{Invariants preserved by the Paxos algorithm}
...
\item\label{paxos-bprom} $\bprom{i}{a}{b}{b'}$ only if $e(b) \le e(i)$, $b'
  \prec b$, $\acc{i}{a}{b'}$, and $b'$ is the greatest such ballot, in the
  sense that $\neg \acc{i}{a}{b''}$ for all $b''$ having $b' \prec b'' \prec
  b$.

\item\label{paxos-prop} $\prop{i}{b}$ only if there is a $q \in \QI{e(b)}$ such
that for every $a \in q$ one of the following holds:
%
\begin{itemize}
...
and if $P = \{ b' \mid \exists a \in q. \bprom{i}{a}{b}{b'} \} \ne \varnothing$
then $v_i(b) = v_i(\mathrm{max}(P))$.


\item \label{paxos-acc} $\acc{i}{a}{b}$ only if $\prop{i}{b}$.

\item \label{paxos-chosen} $\chosen{i}{b}$ only if for each $j < i$ there is a
  $b'$ with $\chosen{i-1}{b'}$, $e(i) \le e(b) + 1$, and there is a quorum $q
  \in \QII{e(i)}$ with $\acc{i}{a}{b}$ for every $a \in q$.
...
The invariants listed in figure \ref{paxos-invariants-figure} are roughly the
...
progress by eventually choosing a value for each instance. It is impossible to
show this deterministically in a fully asynchronous
system\cite{flp-impossibility} but as with earlier presentations of
Paxos\cite{paxos-made-simple} here liveness can be proved by assuming that a
...
suitable ballots since a proposed ballot must not belong to an era which is too
large. This is because if ${e(b) > e(i)}$ then $\prop{i}{b}$ may never be sent
...
large enough to be accepted but which still belongs to the correct era, or,
more precisely, for each ballot $b$, each era $e \ge e(b)$ and each node $a$
...
imply that certain eras would only have finitely many ballots. Instead an
...
e$ and $\owner{\langle e, n, a \rangle} = a$. This is the approach used in
Egalitarian Paxos\cite{egalitarian-paxos} in which eras are known as
\textit{epochs} but this terminology is avoided here to prevent confusion with
the epochs (views, terms, \ldots) of leader-election protocols (e.g.
\cite{omega-meets-paxos}) which track the current leader rather than the
...
\begin{theorem}[Liveness] Given that there is eventually a non-faulty
  distinguished node $\ell$ which is the only node that may emit prepare
  messages, and given that for every instance there is eventually at least one
  value to propose, then eventually a value is chosen for every instance.
\end{theorem}
...
cluster into two parts between which it has a \textit{casting vote}: it finds
$q \in Q_{e(b)}$ and $q' \in Q_{e(b)+1}$ having $q \cap q' = \{\owner{b'}\}$
and broadcasts $\prep{b'}$ to all nodes in $q'$ except itself. While it is
waiting for the promises to arrive in response to this broadcast, $\owner{b}$
continues to make progress choosing instances of ballot $b$ using the quorum $q
\in Q_{e(b)}$.  Eventually when promises have arrived from every node in $q'$
...
\item The second part of this process requires the existence of a node
  ...
  over the leadership of the cluster to $\owner{b'}$. If there is no node with
  a casting vote then the broadcast of $\prep{b'}$ will prevent further
  progress in era $e(b)$. Again, this preserves consistency and liveness but
...
\{ a_1, a_2, \ldots \}$ then define weight functions \[\wl{n}(a) =
...
this correlation may be costly.  In more detail, a na\"ive approach to swapping
a node $a_{\textrm{old}}$ for a replacement $a_{\textrm{new}}$ in a three-node
cluster using unweighted majorities would be to perform the configuration
changes given by this sequence of weight functions: \[\begin{array}{rcccc}
%
