...
dct25-a1vwi@mythic-beasts.com)}% <-this % stops a space
\thanks{\copyright~2016-7 Tracsis plc. This work is licensed under the
...
\markboth{Unbounded Pipelining in Dynamically Reconfigurable Paxos Clusters --- \gitid}%TODO Journal of \LaTeX\ Class Files,~Vol.~14, No.~8, August~2015}%
{Turner: \mytitle}
...
Bortnikov et al. \cite{bortnikov} implemented a reconfigurable RSM using static
RSMs which may pass responsibility to each other in a manner that is similar to
that of Stoppable Paxos. They improve the performance of their system compared
with Stoppable Paxos by allowing each RSM to speculatively start executing
commands before the transfer of responsibility has been fully agreed.
...
discussed in section \ref{per-phase-quorums} below. In particular, the quorums
need not all mutually intersect, as discovered independently by Howard, Malkhi
and Spiegelman \cite{flexible-paxos}.
...
most one so an era may be skipped at instance $i+1$ by setting $e(i+1) = e(i) +
...
for all $a$ then $M(w) \frown M(w')$.  \end{lemma}

\begin{proof} By lemma \ref{weights-nearly-equal-factors}, since $\sum_{a \in
\mathbb A} |k'w'(a) - kw(a)| = 0$.  \end{proof}
