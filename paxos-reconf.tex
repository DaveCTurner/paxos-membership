...
\begin{abstract}Distributed consensus is an essential building block for
  creating fault-tolerant systems. When equipped with a consensus algorithm a
  distributed system can act as a replicated state machine (RSM), duplicating
  its state across a set of redundant components to avoid the failure of any
  single component leading to a system-wide failure. Paxos and Raft are
  ...
  administrative tasks. Paxos can achieve high performance by pipelining
  (starting work on new requests before existing requests have completed) but
  typically bounds the length of the pipeline to ensure consistency during
  reconfiguration. Raft also supports pipelining and imposes no such bound on
  concurrent requests, preserving consistency instead by restricting which
  reconfigurations may be performed. This article shows how to extend Paxos to
  support a more general form of reconfiguration which subsumes the original
  bounded-pipeline approach as well as Raft-like fully-concurrent
  reconfigurations and more besides.
...
\IEEEPARstart{D}{istributed} systems are systems built from collections of
...
deterministic state machine is replicated across a set of distinct nodes in a
way that ensures each node receives the same sequence of commands. Being
deterministic, each node computes the same state if they all receive the same
inputs.

In order to arrange for each node to receive these commands the system must
achieve consensus on (or \textit{choose}) a consistent sequence $v_0, v_1,
\ldots$ of values. This is the \textit{distributed consensus problem} which has
been thoroughly studied and Paxos \cite{part-time-parliament} and
Raft\cite{raft} are two algorithms that have been developed to solve it. They
guarantee system-wide \textit{consistency}, in the sense that the values chosen
are guaranteed to be equal across the whole system even in the presence of
...
adding or removing nodes while it is running. This allows the components of the
system to be repaired or replaced without needing to take the whole system
offline. It is crucial that all participating nodes agree on the cluster
configuration, and this can be achieved by tracking the configuration with a
component of the RSM itself, and using the consensus algorithm to choose
special reconfiguration commands when a configuration change is desired.
...
essentially runs an infinite sequence of Synod protocols in parallel, using
uniformity of the instances to do so without requiring an infinite amount of
time.  It starts by running phase I of all instances at once and then runs
...
Both algorithms support high throughput by allowing for \textit{pipelining}
\cite{smart} whereby an instance may start running before all previous
...
a value may only be proposed once a quorum of nodes are ready for it, but the
quorums change as values are chosen and the state of the RSM changes.  Paxos
implementations typically solve this problem by limiting the length of the
pipeline to some $\alpha > 0$ and requiring that a configuration change chosen
at instance $i$ does not take effect until at least instance $i + \alpha$,
which means that when a value is proposed there can be no as-yet-undecided
configuration change in the pipeline that could cause a different value to be
chosen. Raft imposes no limit on the number of concurrently-running instances
and instead ensures consistency by restricting which reconfigurations may
occur, and then performs sequences of these restricted reconfigurations in
order to achieve an arbitrary reconfiguration.

The choice of the pipeline length parameter $\alpha$ must be made carefully. If
it is too small then the system may suffer from poor performance due to lack of
parallelism, but if it is too large then configuration changes can be
unreasonably expensive to complete. It would be better to be able to vary the
value of $\alpha$ as the system is running rather than having to choose it
up-front, but there seems to be no published method for doing so safely. It
would be better still to be able to impose this limit only when a configuration
change is in progress and allow an unbounded pipeline while the configuration
is static. The need for any such limit seems
...
\ref{configuration-changes} below. In particular, this result unifies and
generalises the reconfigurations supported by both Raft and Dynamic
Paxos\cite{cheap-paxos} as shown in section
...
\ref{types-of-configuration-change} it is shown how this work generalises the
existing reconfiguration processes supported in Paxos and Raft.  Reworked
proofs are included in section \ref{liveness} and appendices \ref{synod-safety}
and \ref{paxos-invariants} and differences from the original are discussed
throughout.
...
details on the reconfiguration scheme used in their Chubby system.
...
one starts\footnote{TODO mention this again in conclusion}.
...
unbounded pipeline throughout the reconfiguration process by instead limiting
the reconfigurations that can be performed to be, in a sense, sufficiently
small that reconfiguration remains safe.
...
Reed and Junqueira \cite{zab} initially publised Zab without reconfiguration
...
\ref{synod-safety} from which it follows that consistency is guaranteed in the
sense that if $\chosen{}{b}$ and $\chosen{}{b'}$ then $v(b) = v(b')$ as shown
in theorem \ref{synod-safety-theorem}.
...
nodes, where ``sufficiently many'' is a property of the configuration of the
system that is defined precisely in terms of sets of quorums of nodes.
...
The phase-I and phase-II quorums are chosen so as to always contain at least one
node in common, but may vary depending on the ballot $b$ and the phase as
...
expensive to transfer between nodes because they carry a large quantity of
data.
...
hashes, but this idea can be taken a step further and the values can be
...
\succ b_2$, as described in invariant \ref{synod-quorums} of appendix
\ref{synod-safety}. This weaker invariant allows more general reconfigurations
to take place as described in section \ref{configuration-changes} below.
...
respectively the sets of phase-I and phase-II quorums in use.
...
In more detail, configurations are tracked by a component of the RSM whose
state is the finite sequence $\langle \QI{0}, \QII{0} \rangle, \ldots, \langle
\QI{e_\mathrm{max}}, \QII{e_\mathrm{max}} \rangle$ and the era numbers of
instances are tracked by a state machine whose state is the finite
nondecreasing sequence $e(0), \ldots, e(i_\mathrm{max})$. The transitions that
affect these sequences may append one or more elements, but may not change any
existing elements. The sequences are always long enough to make progress in the
sense that if $\chosen{i}{b}$ then they satisfy $i_\mathrm{max} > i$ and
$e_\mathrm{max} \ge e(i_\mathrm{max})$.

In contrast, era numbers $e(b)$ for ballots $b$ are fixed in advance and not
chosen by consensus.
...
with promises. Unlike in the original proof, here there is an upper bound on
suitable ballots, because if ${e(b) > e(i)}$ then $\prop{i}{b}$ may never be
sent and nor may $\prop{i}{b'}$ for any $b' \succ b$ since $e(\cdot)$ is
nondecreasing. Therefore here $\ell$ must be able to choose a ballot that is
large enough but not too large.  More precisely, this requires that for each
ballot $b$, each era $e \ge e(b)$ and each node $a$ there must be a ballot $b'
\succ b$ having $e(b') = e$ and $\owner{b'} = a$.
...
current era are themselves agreed by consensus this kind of promise risks
preventing any further progress from being made.
...
Ph.D. degree from the University of Cambridge in 2004 and 2009 respectively.
