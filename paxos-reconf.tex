...
choose it up-front, and better still to be able to impose this limit only when
a configuration change is in progress and allow an unbounded pipeline while the
configuration is static\cite{reconfiguring-a-state-machine}. That said, it
seems even more elegant to be able to avoid imposing any limit at all even
...
Then, a configuration change is proposed at instance $i+3$. This is a
particular value $v_{i+3}(b)$ which, when chosen, appends the next
configuration $\langle \QI{e+1}, \QII{e+1} \rangle$ to the configuration
sequence, sets $e(i+4) = e(i+5) = \ldots = e+1$, and increases $e_\mathrm{max}$
and $i_\mathrm{max}$ accordingly.

On the receipt of each proposal the node $a_2$ responds with an acceptance,
which when received by $\ell$ allows it to deduce that each value is chosen
because $\{ \ell, a_2 \} \in \QII{e}$. In particular when $\chosen{i+3}{b}$ it
updates its configuration state machine, taking the system out of normal
running because now $e(i_\mathrm{max}) = e + 1 \ne e(b)$.  In order to bring
the system back into normal running, the leader must choose a ballot $b'$
...
and $a_2$ started to accept these proposals. Even though these instances belong
to the new configuration, when the leader receives $\acc{i+k}{a_2}{b}$ it may
still safely deduce $\chosen{i+k}{b}$ since $e(i+k) = e + 1 \le e(b) + 1$ for
$k \in \{4,5,6\}$, and $\{ \ell, a_2 \} \in \QII{e+1}$. It is important to
notice that the leader is now using $\QII{e+1}$ and not $\QII{e}$ to determine
when ballots are chosen. The horizontal dashed line shows the point in time at
which the leader moves from era $e$ to era $e+1$.
...
reconfiguration performed in a Stoppable Paxos cluster. This variant of Paxos
allows for an unlimited number of proposals to run in parallel within each
configuration, and permits out-of-order execution, so so in this illustration
the stopping command is proposed at instance $i+3$ before proposals are made
for the two preceding instances.
By selecting instance $i+3$ for the reconfiguration, the operator is limiting
the system to service at most 2 more client requests before the reconfiguration
completes. As in the illustration of Dynamic Paxos above, the operator's choice
is too conservative so the remaining two instances are exhausted before phase I
is completed at ballot $b'$, which causes the system to have to pause its
