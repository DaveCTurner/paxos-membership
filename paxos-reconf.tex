...
value $v_i \in \mathbb V$ when $\ge f+1$ (i.e. a majority) of the nodes agree.
The (nonempty) sets of nodes involved in reaching consensus are known as
...
weaker assumption that $\QI{}(b_1) \frown \QII{}(b_2)$ when $\prop{}{b_1}$,
...
term is not used here to avoid confusion with the epochs of Raft which track
the current leader rather than the current configuration.
...
stale. As with Vertical Paxos these values are managed by an external oracle.
