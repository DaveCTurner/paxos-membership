...
is running, and that Paxos also supports an unlimited-length pipeline by
...
and set $Q_{e+1}$ to be the \textit{joint configuration} of $Q_{e}$ and $Q'$:
\[Q_{e+1} = \{ q \cup q' \mid q \in Q_{e}, q' \in Q' \}.\] Intuitively, while
this joint configuration is in use all decisions must be agreed by quorums of
both the old and new configuration. Certainly this satisfies that \[Q_{e}
\frown Q_{e + 1} \frown Q_{e+1} \frown Q_{e + 2} \frown Q_{e + 2}\] as
required.
...
(i.e.  $Q_{e} \not\frown Q' \frown Q'$) then they can set $Q_{e+2} = Q'$ and
pick an appropriate $Q_{e+1}$ which satisfies that \[Q_{e} \frown Q_{e + 1}
\frown Q_{e+1} \frown Q'.\] For instance $Q_{e+1}$ could be $\varnothing$ or
could be the joint configuration of $Q_{e}$ and $Q'$.  However, re-running
phase I can only be delayed if the era increases by $1$ and in this situation
the era increases by $2$, so the new phase I must be complete before any new
phase II instances can be started.  As in Dynamic Paxos the operator chooses an
...
in the hope that this is long enough to have completed the new phase I. Unlike
in Dynamic Paxos there is only a need to limit the pipeline length while a
...
pipeline throughout, and implements a basic kind of stoppability similar to
those in Stoppable Paxos\footnote{TODO cite, and more detail}.  It achieves
equivalent goals to those of Vertical Paxos\footnote{TODO cite} except that
here there is no requirement for a separate oracle to manage the configuration
of the system, which avoids the problem that for full resilience it must be
possible to reconfigure the oracle itself, which requires another oracle and so
...
defining $e(i)$ similarly seems feasible but the details have so far proved
elusive.
