...
with the consensus algorithm itself using a state machine whose transitions
...
consistency is nontrivial because a value may only be proposed once a quorum of
nodes are ready for it, but whether a set of nodes is a quorum or not may
change as earlier values are chosen. Paxos typically solves this problem by
limiting the length of the pipeline to some $\alpha > 0$ and requiring that a
configuration change chosen at instance $i$ does not take effect until at least
instance $i + \alpha$.  Raft imposes no such limit on the number of
concurrently-running instances and instead ensures consistency by only allowing
certain reconfigurations to take place, and then performs sequences of these
...
they are satisfied for any value function $v'$ that agrees with $v$ on proposed
ballots, i.e. having $v(b) = v'(b)$ only whenever $\prop{}{b}$. Since only
$\owner{b}$ may propose $b$, if it has not yet proposed a value for $b$ it can
...
for all $b_1$ and $b_2$ since all majorities intersect. The proof of theorem
...
weaker assumption that $\QI{}(b_1) \frown \QII{}(b_2)$ only need hold when
...
$e(i)$ records which quorums may be used to decide that $\chosen{i}{b}$ in
...
$b$ in instance $i$ from a quorum of nodes in $\QI{e(b)}$, and similarly a node
may decide $\chosen{i}{b}$ only if ${e(i) \le e(b)+1}$ and it has received
$\acc{i}{a}{b}$ from a quorum of nodes in $\QII{e(i)}$.
...
e(i_\mathrm{max})$. Era numbers $e(b)$ for ballots $b$ are fixed in advance and
not chosen by Paxos.

\subsection{Liveness}\label{liveness}

As with earlier presentations of Paxos\cite{paxos-made-simple},
...
nondecreasing.  Therefore here $\ell$ must be able to choose a sufficiently
large ballot in a sufficiently small era. More precisely, this requires that
for each ballot $b$, each era $e \ge e(b)$ and each node $a$ there must be a
ballot $b' \succ b$ having $e(b') = e$ and $\owner{b'} = a$.  This means that
ballot numbers cannot be simple integers, because this would imply that all but
one era has only finitely many ballots.  Instead one could, for example, let
...
Paxos\cite{egalitarian-paxos} in which eras are known as \textit{epochs}: this
word is not used here to avoid confusion with the epochs (terms, views, \ldots)
of the leader-election protocol which track the current leader rather than the
...
and liveness but may limit performance.
%
...
replacing a node in a three-node cluster would be to perform this sequence of
configuration changes: \[\begin{array}{rcccc}
%
...
correlated failure of two nodes would prevent further progress.  It also has no
node with a casting vote in era $e+1$. On the other hand the following sequence
allows $a_{\textrm{old}}$ and $a_{\textrm{new}}$ to share infrastructure
without extra risk, and both $a_1$ and $a_2$ have casting votes throughout:
\[\begin{array}{rcccc}
%
\textrm{node}&a_{\textrm{old}}&a_{\textrm{new}}&a_1&a_2 \\
%
...
reconfigurations\cite{raft} which can be performed with an unlimited
pipeline throughout, and implements the ``brick-wall'' stopping scheme as in
Stoppable Paxos\cite{stoppable-paxos}. Unlike in Stoppable Paxos, here an
...
oracle and so on \textit{ad infinitum}. Egalitarian
Paxos\cite{egalitarian-paxos} also defines a nondecreasing $e(b)$ for each
ballot $b$ to ensure that ballots are not chosen when their configuration is
stale, but as with Vertical Paxos these values are managed by an external
oracle.
...
leader. In such a situation consensus-based approaches such as the one
presented here would fail to make any further progress, whereas one based on
eventual consistency could be reconfigured into one in which a leader may be
elected and thus in which further progress can be made.  It is certainly
possible to modify the approach presented here to allow the configurations
themselves to be defined in an eventually-consistent manner; defining $e(i)$
similarly seems feasible too but the details have so far proved elusive.
