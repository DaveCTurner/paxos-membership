...
\begin{abstract}Distributed consensus is an essential building block for
  ...
  its state across a number of redundant components to avoid the failure of any
  single component leading to a system-wide failure. Paxos and Raft are two
...
\IEEEPARstart{D}{istributed} systems can tolerate faults if they are built from
redundant components such that certain sets of components may fail without
causing a system-wide failure. A replicated state machine (RSM) is a style of
fault-tolerant distributed system in which a deterministic state machine is
replicated across a number of redundant nodes. If each node receives the same
sequence of commands then, being deterministic, they all compute the same state.

In order to arrange for each node to receive the same sequence of commands the
system must achieve consensus (or \textit{choose}) a consistent sequence $v_0,
v_1, \ldots$ of values, which is known as the \textit{distributed consensus
problem}. Paxos \cite{part-time-parliament} and Raft\cite{raft} are two
algorithms that solve this problem.  They guarantee system-wide
...
should be tolerated, and consensus is achieved on a value $v_i$ when $\ge f+1$
(i.e. a majority) of the nodes agree.  The (nonempty) sets of nodes involved in
reaching consensus are known as \textit{quorums} and the set of quorums in use
is known as the \textit{configuration} of the cluster.
...
configuration change in the pipeline.  Raft imposes no limit on the number of
concurrently-running instances and instead ensures consistency by restricting
which reconfigurations may occur, and then performs sequences of these
restricted reconfigurations in order to achieve an arbitrary reconfiguration.
...
differences from the original are discussed throughout. We begin with a recap
of the Synod algorithm in section \ref{synod-text} and follow this by covering
the full Paxos algorithm in section \ref{paxos-text}, then in section
\ref{types-of-configuration-change} it is shown how this work generalises the
existing reconfiguration processes supported in Paxos and Raft.
...
details on the scheme that they use.
...
An interesting alternative approach was proposed by Jahl and Meling
\cite{async-reconfig} which uses eventual consistency rather than consensus to
determine the configuration of the system, and therefore supports
...
may be large or expensive to send between nodes so an efficient implementation
must minimise the need to transport values as much as possible; the messages
involved in the protocol are very small if the values are removed.
